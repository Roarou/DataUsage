...
def parse_args():
    ...
    parser.add_argument('--npoint', type=int, default=7500, help='Point Number [default: 4096]')
    ...
    for folds in range(10):
        ...
        def weights_init(m):
            ...
            if classname.find('Conv2d') != -1:
                ...
                torch.nn.init.constant_(m.bias.data, 0.0)
        print('gogogo')
        experiment_dir = r'C:\Users\cheg\PycharmProjects\DataUsage\Model\log\sem_seg\2023-12-19_14-41\checkpoints\model.pth'

        print(os.path.exists(experiment_dir))
        try:
            checkpoint = torch.load(experiment_dir)
            print(checkpoint)
        ...
        with torch.no_grad():
            ...
            log_string(iou_per_class_str)
            log_string('Test mean loss: %f' % (loss_sum / num_batches))
            log_string('Test accuracy: %f' % (total_correct / float(total_seen)))

            if mIoU >= best_t_iou:
                best_iou = mIoU
                best_fold = folds
            log_string('Mean mIoU: %f' % mIoU)
            log_string('Best test mIoU: %f' % best_iou)
            log_string('Best fold: %d'% best_fold)
