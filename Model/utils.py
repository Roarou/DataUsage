...
class PointNetSetAbstractionMsg(nn.Module):
    """
    PointNet Set Abstraction with Multi-Scale Grouping (MSG) Layer.

    Parameters:
        npoint: int
            The number of points to sample in farthest point sampling.
        radius_list: List[float]
            List of radii to define the range of each group.
        nsample_list: List[int]
            Number of points to sample in each group.
        in_channel: int
            Number of input channels.
        mlp_list: List[List[int]]
            List of MLP layers for each scale group.

    Returns:
        new_xyz: Tensor
            The new coordinates of the sampled points.
        new_points_concat: Tensor
            The concatenated features for the next layer.
    """
...
class PointNetFeaturePropagation(nn.Module):
    ...
    It takes in the coordinates and features from two consecutive layers
    and interpolates the features of one layer to another, followed by
    applying MLPs to the concatenated features.

    Parameters:
        in_channel: int
            Number of input channels.
        mlp: List[int]
            List of output channels in the MLP applied to the features.

    Returns:
        new_points: Tensor
            The new features after feature propagation and MLP.
    """
