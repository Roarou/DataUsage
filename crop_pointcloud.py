...
"""
    ...
    point_cloud.colors = o3d.utility.Vector3dVector(
        np.concatenate([point_cloud.colors, np.tile(new_point_color, (1, 1))]))
    ...
    point_cloud.colors = o3d.utility.Vector3dVector(
        ...
        """
        # Specimen 2
        min_bounding_box_coords = np.array(
            [min_x - (300 - abs(abs(min_x) - abs(max_x))) / 2, min_y - (300 - abs(abs(min_y) - abs(max_y))) / 2,
             min_z - (0.3 * abs(min_z))])
        max_bounding_box_coords = np.array(
            [max_x + (300 - abs(abs(min_x) - abs(max_x))) / 2, max_y + (300 - abs(abs(min_y) - abs(max_y))) / 2, max_z])
...
"""
    ...
    o3d.visualization.draw_geometries([point_cloud, coord_frame, bbox])
...
def bbox_point_cloud(input_path: str, path_pose):
    # Read point cloud from file
    point_cloud = o3d.io.read_point_cloud(input_path)
    ...
    # Update the color of the point cloud
    point_cloud.colors = o3d.utility.Vector3dVector(black_color)
    bbox = o3d.geometry.AxisAlignedBoundingBox(min_bounding_box_coords, max_bounding_box_coords)
